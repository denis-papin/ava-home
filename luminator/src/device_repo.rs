...
fn factory(message_type: &str) -> MessageEnum {
    let path_to_json = format!(r"C:\Users\gcres\Projects\wks-ava-home\ava-home\luminator\resources\{}.json", message_type);
    let  object_json= fs::read_to_string(path_to_json).unwrap();
    let message : MessageEnum =  serde_json::from_str(object_json.as_str()).unwrap(); // TODO
    message
}
...
pub (crate) fn build_device_repo() -> HashMap<String, Arc<RefCell<GenericDevice<MessageEnum>>>> {
    ...
    let ALL = vec![DeviceDefinition {
        family: ZIGBEE_FAMILY.to_string(),
    ...
    let dev_list: Vec<GenericDevice<MessageEnum>> = ALL.iter().map(|def| GenericDevice::new(&def.family, &def.name, factory(&def.message_type), def.process_same_message) ).collect();
