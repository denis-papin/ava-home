...
const CURRENT_REGULATION_MAP_SQL: &str =
    "SELECT id, starting_time, ending_time, end_the_next_day, boost, regulation_map, ts_created
...
pub(crate) async fn get_current_regulation_map(
) -> anyhow::Result<(NaiveTime, NaiveTime, RegulationMapMsg)> {
    let mut params = HashMap::new();
    params.insert(
        "p_localtime".to_owned(),
        CellValue::from_opt_naivetime(Some(Local::now().time())),
    );
    ...
    if sql_result.next() {
        ...
        let starting_time = sql_result
            .get_naivetime("starting_time")
            .ok_or_else(|| anyhow!("Aucune valeur trouvée pour starting_time"))?;
        let ending_time = sql_result
            .get_naivetime("ending_time")
        ...
        let reg_map: RegulationMapMsg = serde_json::from_value(reg).map_err(|e| {
            anyhow!(
                "Erreur lors de la désérialisation de la regulation_map: {}",
                e
            )
        })?;
